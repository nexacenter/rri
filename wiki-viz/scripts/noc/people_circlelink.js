function doCircle(mode) {
    ...
    var findPeople = ["", "SELECT DISTINCT ?name", "WHERE {", "?person a foaf:Person .", "?person <http://www.w3.org/2000/01/rdf-schema#label> ?name .", "?person <http://xmlns.com/foaf/0.1/topic_interest> ?topic .", "}"].join(" ");

    var findTopic = ["", "SELECT DISTINCT ?name ?t", "WHERE {", "?person a foaf:Person .", "?person <http://xmlns.com/foaf/0.1/topic_interest> ?t .", "?person <http://www.w3.org/2000/01/rdf-schema#label> ?name .", "}"].join(" ");

    $.ajax({
        dataType: "jsonp",
        url: endpoint + "&query=" + encodeURIComponent(findPeople) + "&format=json",
